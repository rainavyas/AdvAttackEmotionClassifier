...
if __name__ == '__main__':
    ...
    commandLineParser.add_argument('--layer_num', type=int, default=12, help="layer to investigate")
    commandLineParser.add_argument('--num_points_train', type=int, default=6000, help="number of data points to use train")
    commandLineParser.add_argument('--num_points_test', type=int, default=2000, help="number of data points to use test")
    commandLineParser.add_argument('--num_points_val', type=int, default=200, help="number of test data points to use for validation")
    ...
    embeddings = batched_get_layer_embedding(original_list, handler, tokenizer, device)
    original_pca_comps = get_pca_principal_components(v, correction_mean, embeddings, num_comps, start)

    embeddings = batched_get_layer_embedding(attack_list, handler, tokenizer, device)
    attack_pca_comps = get_pca_principal_components(v, correction_mean, embeddings, num_comps, start)

    labels = torch.LongTensor([0]*original_pca_comps.size(0)+[1]*attack_pca_comps.size(0))
    X = torch.cat((original_pca_comps, attack_pca_comps))
